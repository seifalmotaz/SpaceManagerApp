import 'package:flutter/material.dart';

class BaseColors {
  static const Color white = Colors.white;
  static const Color whiteBased = Color(0xFFe4e6eb);
  static const Color primary = Color(0xFFfa735a);
  static const Color lightPrimary = Color(0xFFf8e9e1);
  static const Color darkPrimary = Color(0xFF981804);
  static final Color darkText = Colors.grey.shade900;
  static const textColor = Color(0xFF37474F);
  static const semiTextColor = Color(0xFF455A64);
}

class ColorPalette {
  static const Color white = Colors.white;
  static const Color whiteBased = Color(0xFFe4e6eb);
  //
  static const Color dark = Color(0xFF191919);
  //
  static const Color navy = Color(0xFF2D4263);
  //
  static const Color bittersweet = Color(0xFFfa735a);
  static const Color lightBittersweet = Color(0xFFf8e9e1);
  //
  static const Color beige = Color(0xFFECDBBA);
}

const Color colorWhite = Colors.white;
const Color colorWhiteBased = Color(0xFFe4e6eb);
const Color colorWhiteBased2 = Color(0xFFf6f8fa);

const Color colorDarkLight = Color(0xFF252525);
const Color colorDarkLighter = Color(0xFF303030);
const Color colorDarkLightest = Color(0xFF3f3f3f);

const Color colorNavy = Color(0xFF2D4263);

const Color colorBittersweet = Color(0xFFfa735a);
const Color colorLightBittersweet = Color(0xFFf8e9e1);

const Color colorMaldives = Color(0xFF00B6DB);
// dark Maldives
const Color colorKingfisherSheen = Color(0xFF007FA4);

const Color beige = Color(0xFFECDBBA);

const colorText = Color(0xFF37474F);
const colorSemiText = Color(0xFF455A64);
